...
export function LocationFinalization() {
  ...
  const pendingApprovals = APPROVALS.filter(a => a.module === 'Location Finalization' && a.status === 'Pending' && a.assignedTo === currentUser.id);
      ...
      <div className="p-3 sm:p-6 space-y-4 sm:space-y-5">
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1 relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search markets..." className="w-full pl-9 pr-4 py-2.5 bg-white border border-gray-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-200 shadow-sm" />
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  {['Market ID', 'Pin Code', 'Market Name & Address', 'Category', 'Households', 'Ownership', 'Docs', 'Day / Time', 'Outlets', 'Finalized Rent', 'Status', 'Action'].map(h => (
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-3">
                        <span className={`text-[11px] font-bold px-2.5 py-1 rounded-lg border ${rowOverdue ? 'border-red-400 text-red-600 bg-red-50' : 'border-blue-200 text-blue-600 bg-blue-50'}`}>{f.id}</span>
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-[11px] px-2.5 py-1 rounded-lg font-semibold ${STATUS_STYLE[f.status]}`}>
                      ...
                      <td className="px-4 py-3">
                        {canApprove && pending ? (
                          ...
                          <span className="text-[11px] px-3 py-1.5 rounded-lg font-semibold bg-emerald-500 text-white whitespace-nowrap">PUSHED TO ONBOARDING</span>
                        ) : (
